<template>
  ...
  >
    <b-row>
      <b-col>
        <b-img fluid :src="'./uploads/' + `${imgUrl}`"> </b-img>
      </b-col>
    </b-row>
    <hr />
    <b-container fluid class="item-info">
      <b-row class="p-2 text-left">
        <b-col cols="12">Delivery Type: {{ item.deliveryType }}</b-col>
        <b-col cols="12" v-if="item.deliveryType == 'Meet Up'">
          Location: {{ item.location }}
        </b-col>
      </b-row>
      <b-row>
        <b-col class="text-right">
          State: <strong>{{ item.state }} </strong></b-col
        >
      </b-row>
    </b-container>
    ...
    <b-modal
      ...
      centered
      :title="`${item.name}`"
      :ok-title="this.$store.state.isMarketItems ? 'I want this!' : 'Remove item'"
      cancel-title="Close"
      ok-variant="success"
